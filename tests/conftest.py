...
from dvm_sdk import DvmClient, AsyncDvmClient, DefaultAioHttpClient
from dvm_sdk._utils import is_dict
...
logging.getLogger("dvm_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[DvmClient]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with DvmClient(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncDvmClient]:
